# FeedテーブルからRssBucket依存を排除する計画 (v1)
...
- 既存 SQLite DB の移行手順とサポート用スクリプト整備。
...
- 既存 DB との互換を保つため、`infra.storage.rds` に軽量マイグレーション関数 (例: `migrate_feed_schema_v2`) を追加し、
  1. 一時テーブルを作成 (`feed_item_v2`)、
  2. 旧テーブルから必要なカラムをコピー (`created_at`/`updated_at` には `datetime.utcnow()` を利用)、
  3. 旧テーブル/インデックスを削除、
  4. 新テーブルを `feed_item` へリネーム、という手順を自動化する。
  - SQLite でカラム削除がサポートされないケースを想定し、上記ロールフォワード方式を採用する。
  - `rss_bucket` テーブルは `DROP TABLE IF EXISTS rss_bucket` を発行して削除する。

### 3.2 サービス/コマンド層
- `create_feed` の引数から `bucket_id` を除外し、呼び出し側が URL・タイトル・HTTP ステータス・公開日時のみで FeedItem を生成できるようにする。
- `store_feed` は以下を担う:
  - Upsert 前に `initialize_database()` と `migrate_feed_schema_v2()` を呼んでスキーマ整合性を確保。
...
- 実装完了後、`AGENTS.md` に今回のスキーマ変更とテーブル移行コマンドを追記する。
...
## 4. データ移行計画
- 既存ユーザー向けに `scripts/migrate_feed_v2.py` (新設の想定) を提供し、`just migrate-feed-v2` のようなエントリから実行できるようにする。
  - 旧 `bucket_id` カラムは破棄されるため、必要に応じて別途バックアップを取得する旨を README/AGENTS に案内する。
  - マイグレーションが複数回呼ばれても冪等となるよう、実行前に `PRAGMA table_info('feed_item')` でカラム存在チェックを行う。
- 既存 DB が不要な場合は `data/datadoggo.db` を削除して再生成する運用も記載する。
...
## 6. リスクと対応策
- SQLite のバージョンによっては `ALTER TABLE DROP COLUMN` が利用できないため、マイグレーションロジックを十分に検証する必要がある。
  - 対応: 一時テーブルを用いた移行をデフォルトにし、単体テストでファイルベース SQLite を使って検証。
