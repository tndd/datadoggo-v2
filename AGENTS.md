...
## プロジェクト構成とモジュール
リポジトリの中核コードは `src/` 配下にまとまり、ドメインロジックは `src/domain/`、外部I/Oは `src/infra/`、業務フローは `src/workflow/` に配置されています。**テストは `tests/` ディレクトリに分離され、`test_*.py` 形式で実装されています。** 永続化に必要なデータは `data/`、追加ドキュメントは `docs/` を参照してください。
...
## テスト方針
テストは `tests/` ディレクトリに配置し、関数形式（`def test_*()`）で実装します。目的と検証観点を docstring に記述し、ケース数は対象ごと最大5件に絞ります。自動化チェックは `ruff`→`pyright`→`pytest` の順で実行し、警告も必ず解消します。外部通信を伴う場合は `@pytest.mark.online` を付け、既定のコマンドでは除外される前提です。

### テスト構成
- `tests/` 配下のディレクトリ構造は `src/` と対応
- テストファイル名は `test_{モジュール名}.py` 形式
- 共通fixtureは `tests/conftest.py` に配置
- pyfakefs使用時はfs fixtureを引数に追加
- DB操作テストは自動的にインメモリDBを使用
...
- **pytest実行時**: `tests/conftest.py`でインメモリDB `sqlite:///:memory:` を自動設定
